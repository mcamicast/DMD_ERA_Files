...
for k = 1:n_mac
   ...
   if ~isempty(exc_con)
      ...
      if n_smp~=0
         k_smp = find((mac_exc(smp_idx))==k); % k_smp = find(mac_int(mac_exc(smp_idx))==k);
         if ~isempty(k_smp)
      ...
      if n_smppi~=0
         k_smppi = find((mac_exc(smppi_idx))==k); % k_smppi = find(mac_int(mac_exc(smppi_idx))==k);
         if ~isempty(k_smppi)
      ...
      if n_dc~=0
         k_dc = find((mac_exc(dc_idx))==k); % k_dc = find(mac_int(mac_exc(dc_idx))==k);
         if ~isempty(k_dc)
      ...
      if n_st3 ~=0 
         k_st3 = find((mac_exc(st3_idx))==k); % k_st3 = find(mac_int(mac_exc(st3_idx))==k);
         if ~isempty(k_st3)~=0
   ...
   %pss
   if ~isempty(pss_con)
      k_pss = find((mac_pss)==k); % k_pss = find(mac_int(mac_pss)==k);
      s_T4 = 0;
      if ~isempty(k_pss)
         k_p = find(mac_pss==k);
   ...
   %deltaP/omega filter
   if ~isempty(dpw_con)
      k_dpw = find((mac_dpw)==k); % k_dpw = find(mac_int(mac_dpw)==k);
      if ~isempty(k_dpw)
         state(k) = state(k) + 6;
         dpw_state(k)=1;
      end
   ...
   % turbine governors
   if ~isempty(tg_con)
      k_tg = find((mac_tg)==k); % k_tg = find(mac_int(mac_tg)==k);
      if ~isempty(k_tg)
         state(k) = state(k) + 3;
         tg_state(k) = 1;
      end
      k_tgh = find((mac_tgh)==k); % k_tgh = find(mac_int(mac_tgh)==k);
      if ~isempty(k_tgh)
         state(k) = state(k) + 5;
         tgh_state(k) = 1;
      end
...
if n_tcsc~=0
   ...
   n_tcsc_states = sum(state_tcsc); % n_svc_states = sum(state_tcsc);
   n_tcsc1 = ntot + n_svc;
...
n_shtr_states = 0;
if n_shtr~= 0
    state_shtr(1:n_shtr) = ones(n_shtr,1);
    n_shtr_states = sum(state_shtr);
    n_shtr1 = ntot + n_svc + n_tcsc + n_lmod + n_rlmod + n_dcl;
    state(n_shtr1+1:n_shtr1+n_shtr) = ones(n_shtr,1);
end

% TSCR
n_tcsr_states = 0;
if n_tcsr~= 0
    state_tcsr(1:n_tcsr) = ones(n_tcsr,1);
    n_tcsr_states = sum(state_tcsr);
    n_tcsr1 = ntot + n_svc + n_tcsc + n_lmod + n_rlmod + n_dcl + n_shtr;
    state(n_tcsr1+1:n_tcsr1+n_tcsr) = ones(n_tcsr,1);
end
...
if n_wtg~= 0
    ...
    nmass2idx = find(nmasswtg == 2);
    
    Varflagwtg = wtg_con(wtg_ix,52);
    Varflagidx_p1 = find(Varflagwtg == 1);
    Varflagidx_p2 = find(Varflagwtg == 2);
    Varflagidx_p1_p2 = find((Varflagwtg == 1) | (Varflagwtg == 2));
    
    Varflagidx_0 = find(Varflagwtg == 0);
    Varflagidx_p3 = find(Varflagwtg == 3);
    Varflagidx_0_p3 = find((Varflagwtg == 0) | (Varflagwtg == 3));
    
    state_wtg(1:n_wtg) = 12*ones(n_wtg,1); % 12 states if 1 mass model Type 3
    state_wtgnm2 = 0;
    snm2 = zeros(n_wtg,1);
    if ~isempty(nmass2idx)
        state_wtgnm2 = 2*ones(length(nmass2idx),1);
        snm2(nmass2idx) = state_wtgnm2;
    end
    
    state_wtg_rppfr = 0;
    s_rppfr = zeros(n_wtg,1);
%     if ~isempty(Varflagidx_0)
%         state_wtg_rppfr = ones(length(Varflagidx_0),1);
%         s_rppfr(Varflagidx_0) = state_wtg_rppfr;
%     end
    if ~isempty(Varflagidx_0_p3)
        state_wtg_rppfr = ones(length(Varflagidx_0_p3),1);
        s_rppfr(Varflagidx_0_p3) = state_wtg_rppfr;
    end    
    state_wtg_rpwce = 0;
    s_rpwce = zeros(n_wtg,1);
%     if ~isempty(Varflagidx_p1)
%         state_wtg_rpwce = 5*ones(length(Varflagidx_p1),1);
%         s_rpwce(Varflagidx_p1) = state_wtg_rpwce;
%     end
    if ~isempty(Varflagidx_p1_p2)
        state_wtg_rpwce = 5*ones(length(Varflagidx_p1_p2),1);
        s_rpwce(Varflagidx_p1_p2) = state_wtg_rpwce;
    end


%     state_wtg_nfc = 0;
%     s_wtg_nfc = zeros(n_wtg,1);
%     if size(wtg_con,2)>75
%         fcflagwtg = wtg_con(wtg_ix,76);
%         ixfcwtg1 = find(fcflagwtg == 1);
%         if ~isempty(ixfcwtg1)
%             state_wtg_nfc = 3*ones(length(ixfcwtg1),1);
%             s_wtg_nfc(ixfcwtg1) = state_wtg_nfc;
%         end
%         
%     end
    
    n_wtg_states = sum(state_wtg) + sum(state_wtgnm2) + sum(state_wtg_rpwce);% + sum(state_wtg_nfc);
    n_wtg1 = ntot + n_svc + n_tcsc + n_lmod + n_rlmod + n_dcl + n_shtr + + n_tcsr; %  + n_conv;
    state(n_wtg1+1:n_wtg1+n_wtg) = 12*ones(n_wtg,1) + snm2 + s_rppfr + s_rpwce;% + s_wtg_nfc;
end