...
% Author:   Joe H. Chow
% Date:     April 1991
global pruebar % included to test
...
% wtg variables
global wtg_con n_wtg Iswtg wtg_idx wtg_idx2
% TCSR variables
global tcsrf_idx tcsrt_idx n_tcsr X_tcsr Xtcsr_or tap_tcsr phasesh_tcsr
% Shunt reactor (frequency controlled) variables
global Yshtr n_shtr shtr_idx
% Global
global Iccinj n_ccinj ccinj_idx ccinj_idx2
global lmod_sig rlmod_sig % borrar despues de projecto CDC y acomodar bien
...
if ~isempty(load_con)
   ...
   if flag == 0; % initialization
      ...
      % Equivalents of constant impedance
   ...
   if flag == 1 % network interface computation 
      ...
      if n_shtr~=0
         ixshr = shtr_idx;    
         Y22(ixshr,ixshr) = Y22(ixshr,ixshr) - 1i*diag(Yshtr(:,k));% note that Y22 is a local variable
      end
      if n_tcsc~=0
         % note that Y22 is a local variable- hence Y22 only changes inside
         % nc_load (this function)
         j = tcscf_idx; jj = tcsct_idx;
         Y22(j,j) = Y22(j,j) + jay*diag(B_tcsc(:,k));
         Y22(j,jj) = Y22(j,jj) - jay*diag(B_tcsc(:,k));
         Y22(jj,j) = Y22(jj,j) - jay*diag(B_tcsc(:,k));
         Y22(jj,jj) = Y22(jj,jj) + jay*diag(B_tcsc(:,k));
      end
      if n_tcsr~=0
          ixf = tcsrf_idx; ixt = tcsrt_idx;
          Ytcsr_or = 1./Xtcsr_or;
          Ytcsr_new = 1./(Xtcsr_or + X_tcsr(:,k));
          Ytcsr_ch = (Ytcsr_or - Ytcsr_new); % this one because 1/j = -1
          Cch = (1./tap_tcsr).*exp(-1i.*phasesh_tcsr*pi/180);
          
          Y22(ixf,ixf) = Y22(ixf,ixf) + 1i*diag(Ytcsr_ch.*Cch.^2); % change with square
          Y22(ixf,ixt) = Y22(ixf,ixt) - 1i*diag(Ytcsr_ch.*Cch); % change tap
          Y22(ixt,ixf) = Y22(ixt,ixf) - 1i*diag(Ytcsr_ch.*Cch); % change tap
          Y22(ixt,ixt) = Y22(ixt,ixt) + 1i*diag(Ytcsr_ch); % no change
      end
      if n_lmod ~=0
          % ****change accomodate time constant = 0
         ixT0 = abs(lmod_con(:,7))<1e-7;
         lmod_st(ixT0,k) = lmod_con(ixT0,6).*lmod_sig(ixT0,k);
         % end ****change accomodate time constant = 0
         j = lmod_idx;
         Y22(j,j) = Y22(j,j) + diag(lmod_st(:,k));
      end
      if n_rlmod ~=0
         % ****change accomodate time constant = 0
         ixT0 = abs(rlmod_con(:,7))<1e-7;
         rlmod_st(ixT0,k) = rlmod_con(ixT0,6).*rlmod_sig(ixT0,k);
         % end ****change accomodate time constant = 0
         j = rlmod_idx;
         Y22(j,j) = Y22(j,j) + jay*diag(rlmod_st(:,k));
      end
      
      lv_idx = find(abs(V_nc)<=0.5); %lv_idx = find(abs(V_nc)<=0.5);
      hv_idx = find(abs(V_nc) > 0.5); %hv_idx = find(abs(V_nc) > 0.5);
      
      curr_nc = zeros(length(V_nc),1);% * modif
      
      curr_mis = zeros(nload,1);
      curr_load = Y21*psi + Y22*V_nc;
      if ~isempty(hv_idx)
         curr_nc(hv_idx) = -conj((load_pot(hv_idx,1)+load_pot(hv_idx,2)...%* modif curr_nc = -conj((load_pot(hv_idx,1)+load_pot(hv_idx,2)...
            .*abs(V_nc(hv_idx)))./V_nc(hv_idx));
      end
      
      if ~isempty(lv_idx)
         %* modif curr_nc = -conj(diag(load_pot(lv_idx,3)+load_pot(lv_idx,4)))*V_nc(lv_idx);
         curr_nc(lv_idx) = -conj(diag(load_pot(lv_idx,3)+load_pot(lv_idx,4)))*V_nc(lv_idx);
      end
...
%          Vtangwtg = angle(Vwtg);
         ...
         curr_nc(wtg_idx) = Iswtg(:,kk);
         % curr_nc(wtg_idx) = Iswtg(:,kk) + curr_nc(wtg_idx);

      end
      
      if n_ccinj~=0
          ccinj_cur;
          curr_nc(ccinj_idx) = Iccinj_sys_k;
      end
      
      if ~isempty(hv_idx)
         curr_mis(hv_idx)=curr_nc(hv_idx) - curr_load(hv_idx); % current mismatch
      end
      % converts to constant impedance abs(V_nc)<=0.5
      if ~isempty(lv_idx)
%          curr_mis(lv_idx) = -conj(diag(load_pot(lv_idx,3)+load_pot(lv_idx,4)))*V_nc(lv_idx)...
%             - curr_load(lv_idx);
          curr_mis(lv_idx)=curr_nc(lv_idx) - curr_load(lv_idx);
      end
      count = 0;
...
%             vi11 = v_im.*v_im.*real(load_pot(hv_idx,2) + load_wtg) - v2.*imag(load_pot(hv_idx,2) + load_wtg);
%             vi12 = v_re.*v_re.*imag(load_pot(hv_idx,2) + load_wtg) - v2.*real(load_pot(hv_idx,2) + load_wtg);
%             vi21 = -v_im.*v_im.*imag(load_pot(hv_idx,2) + load_wtg) -v2.*real(load_pot(hv_idx,2) + load_wtg);
%             vi22 = v_re.*v_re.*real(load_pot(hv_idx,2) + load_wtg) + v2.*imag(load_pot(hv_idx,2) + load_wtg);
...
%             Vangwtg = angle(Vwtg);
%             wtg_cur;
%            cur_wtg = conj((load_wtg.*abs(Vwtg))./Vwtg); % N
%            curr_nc(wtg_idx) = cur_wtg + curr_nc(wtg_idx); % N
           curr_nc(wtg_idx) = Iswtg(:,kk); % + curr_nc(wtg_idx); % N
%              curr_nc(wtg_idx) = Iswtg(:,kk).*(cos(Vangwtg) + 1i*sin(Vangwtg));
         end
         
        if n_ccinj~=0
            ccinj_cur; % NEEDED for V_nc
            curr_nc(ccinj_idx) = Iccinj_sys_k;
        end
         
         if ~isempty(hv_idx)
            curr_mis(hv_idx) = curr_nc(hv_idx) - curr_load(hv_idx); %* modif curr_mis(hv_idx) = curr_nc - curr_load(hv_idx); % current mismatch
         end
         if ~isempty(lv_idx)
            curr_mis(lv_idx) = curr_nc(lv_idx) - curr_load(lv_idx); %* modif curr_mis(lv_idx) = curr_nc - curr_load(lv_idx);
         end
...
% pruebar = cat(3, pruebar, load_pot) 