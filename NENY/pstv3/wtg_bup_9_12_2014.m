...
global sigpru1_wtg sigpru2_wtg
...
%                 thrshldnfc = ones(n_wtg,1)*[0.025, -0.025];
...
% %         
            Qinput = 0; % Qset_wtg(:,1); % One of the choices GE report page 4.9 top
...
%             
% %             ynmph = busferr_wtg(ixfcwtg2,1);
% %             
% %             fst1_wtg(ixfcwtg2,1) = (p2fcwtg(ixfcwtg2,1) + p3fcwtg(ixfcwtg2,1)).*busferr_wtg(ixfcwtg2,1);
% %             fst1_wtg(ixfcwtg2,1) = ynmph;
...
%             end
            ...
            Vrfq_wtg(:,1) = Vrfq; Vreg_wtg(:,1) = Vreg;
            bus_new = bus;
...
%             
% %================= Freq control Non Min Phase cont over Pset =============%
% % Discontinued
% %             ynmph = (1./p3fcwtg(ixfcwtg2,1)).*(fst1_wtg(ixfcwtg2,k) - p2fcwtg(ixfcwtg2,1).*busferr_wtg(ixfcwtg2,1));
% %             dfst1_wtg(ixfcwtg2,k) = busferr_wtg(ixfcwtg2,1) - ynmph;
% %             dfst2_wtg(ixfcwtg2,k) = (1./p4fcwtg(ixfcwtg2,1)).*(ynmph - fst2_wtg(ixfcwtg2,k));
% %             
% %             sigpru1_wtg(ixfcwtg2,k) = p1fcwtg(ixfcwtg2,1).*fst2_wtg(ixfcwtg2,k);
% % %             Pth_sig_wtg(ixfcwtg2,k)
% %             % Change wref - control limit
% % %             w_sig_wtg(ixfcwtg2,1) = p5fcwtg(ixfcwtg2,1).*busferr_wtg;
% %             sigpru2_wtg(ixfcwtg2,k) = p5fcwtg(ixfcwtg2,1).*busferr_wtg(ixfcwtg2,1);
% %             wsiglim = 0.2; % extra to get to 1.4
% %             ixwsig = (sigpru2_wtg(:,k) > wsiglim)&ixfcwtg2;
% % %             w_sig_wtg(ixwsig,1) = wsiglim;
% %             sigpru2_wtg(ixwsig,k) = wsiglim;
% %             
% %             w_sig_wtg = -sigpru2_wtg(:,k);
%             
% %================= Freq control Switching Control =============%
%             %p1fcwtg % 78
%             %p2fcwtg % 79
% %             sigpru1_wtg(ixfcwtg2,k) = p1fcwtg(ixfcwtg2,1).*busferr_wtg(ixfcwtg2,1);
% %             
% %             ixswc = (swinif_wtg(:,k)<(59.965/60)) & (swinif_wtg(:,k)>(59.955/60))...
% %                 & (dswinif_wtg(:,k)>0) & thrshldnfc(:,1) & t(k)>2;
% %             thrshldnfc(ixswc & ixfcwtg2,1) = 0;
% %             wtg_con(ixswc & ixfcwtg2,78) = p2fcwtg(ixswc & ixfcwtg2);
% %             
% %             if (swinif_wtg(:,k)<(59.965/60)) && (swinif_wtg(:,k)>(59.955/60)) && (dswinif_wtg(:,k)>0) & t(k)>2
% %                 disp('condt')
% % %                 if (dswinif_wtg(:,k)>0) && swcontvar
% % %                     swcontvar = false;
% % %                     wtg_con(wtg_ix,78) = p2fcwtg;% p1fcwtg = wtg_con(wtg_ix,78);
% % %                 end
% %             end
...
% 
% %             Kitrqwtgn = 0.4; % pag. 4.26 GE WTG Modeling-V4.5
% %             Kptrqwtgn = 2.5; % pag. 4.26 GE WTG Modeling-V4.5
% %             Tpcwtgn = 0.05;%0.05; % pag. 4.26 GE WTG Modeling-V4.5
%             
%             Kppwtgn = p4fcwtg;
%             Kpcwtgn = p5fcwtg;
%             Kicwtgn = p6fcwtg;
...
% %             end
%             
% %             wtg_con(ixfcwtg3 & ixbferr2_wtg,19) = Kppwtgn(ixfcwtg3 & ixbferr2_wtg); % PERMANENT Modification
% %             wtg_con(ixfcwtg3 & ixbferr2_wtg,21) = Kpcwtgn(ixfcwtg3 & ixbferr2_wtg); % PERMANENT Modification
% %             wtg_con(ixfcwtg3 & ixbferr2_wtg,20) = Kicwtgn(ixfcwtg3 & ixbferr2_wtg); % PERMANENT Modification
% %             wtg_con(ixfcwtg3 & ixbferr2_wtg,23) = Kptrqwtgn(ixfcwtg3 & ixbferr2_wtg); % PERMANENT Modification
% %             wtg_con(ixfcwtg3 & ixbferr2_wtg,22) = Kitrqwtgn(ixfcwtg3 & ixbferr2_wtg); % PERMANENT Modification
% %             wtg_con(ixfcwtg3 & ixbferr2_wtg,25) = Tpcwtgn(ixfcwtg3 & ixbferr2_wtg); % PERMANENT Modification
%             
...
% %                 disp('dbugg')
% %             end
%             % yfc_zp = p4fcwtg(:,1).*( busferr_wtg(:,1) + fst1_wtg(:,k) ); % not indexed, cos' done it afterwards
...
%             
% %             dfst1_wtg(ixfcwtg4,k) = p4fcwtg(:,1).*( p3fcwtg(ixfcwtg4,1).*(busferr_wtg(ixfcwtg4,1)) - fst1_wtg(ixfcwtg4,k).*p2fcwtg(ixfcwtg4,1) );
% %             dfst2_wtg(ixfcwtg4,k) = p4fcwtg(:,1).*( (busferr_wtg(ixfcwtg4,1)) - p2fcwtg(ixfcwtg4,1).*dfst2_wtg(ixfcwtg4,k) );
% %             sigpru1_wtg(ixfcwtg4,k) =  p1fcwtg(ixfcwtg4,1).*(fst1_wtg(ixfcwtg4,k) + dfst2_wtg(ixfcwtg4,k));
% 
...
%======================== Reactive Power Control =========================%
            ...
            % if varflag == 1 % case when it follows Voltage regulation
            ixvarglaf_p1 = varflag == 1;
            Vrfq = abs( busi_v(bus_int(Vregbuswtg),1) ); % reference voltage - checked in xxx--
            Vrfq = Vrfq(ixvarglaf_p1,1); % reference voltage taken from the load flow
            Vqd = Kqdwtg(ixvarglaf_p1,1).*s7_wtg(ixvarglaf_p1,k); % Figure 4-5 GE report v4.5
            Vreg = abs( busi_v(bus_int(Vregbuswtg),k) ); % Voltage to regulate is defined by user
            Vreg = Vreg(ixvarglaf_p1,1);
            ...
            saux_rpc = Vrfq - s3_wtg(ixvarglaf_p1,k) - Vqd + VQ_sig_wtg(ixvarglaf_p1,k);
            ...
            Qinput = 0; % Qg_wtg; % One of the choices GE report page 4.9 top

            ds7_wtg(ixvarglaf_p1,k) = (1./Tlpqdwtg(ixvarglaf_p1,1)).*(Qinput - s7_wtg(ixvarglaf_p1,k));
...
%             Ipcmd_wtg = Pord_wtg(:,k).*(wtgbase./genbasmva_wtg)./Vtmagwtg; %confirmar el orden de esta ecuacion       
            Ipcmd_wtg = (Pordext_wtg + Pf_sig_wtg(:,k)).*(wtgbase./genbasmva_wtg)./Vtmagwtg; %confirmar el orden de esta ecuacion
...
%             end
            ...
            Vrfq_wtg(:,k) = Vrfq; Vreg_wtg(:,k) = Vreg;
        end
...
%     busferr_wtg = freqrefwtg - (busfreq_wtg + auxsigwini_wtg);
    ...
    auxsigwini_wtg = 0; 
    freqrefwtg = bus_freqf(wtg_idx2,1); % verificar que es la system freq
%     busfreq_wtg = bus_freqf(wtg_idx2,k);% calculated through freqcalc
