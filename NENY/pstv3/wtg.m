...
%*************** --Reactive Power Control Initialization-- ***************%            
            ...
            ixvarglaf_p3 = varflag == 3;% skip int
            ixvarglaf_0_p3 = ixvarglaf_0 | ixvarglaf_p3;
            
            % s6_wtg(ixvarglaf_0,1) = Pg_wtg(ixvarglaf_0,1);
            s6_wtg(ixvarglaf_0_p3,1) = Pg_wtg(ixvarglaf_0_p3,1);
            % Qset_wtg(ixvarglaf_0,1) as before
            
            % if varflag == 1 % case when it follows Voltage regulation            
            ixvarglaf_m2 = varflag == -2;
...
%************************** --------------------- ************************%
            ...
            ixvarglaf_sk = ixvarglaf_p2 | ixvarglaf_p3 | ixvarglaf_m2;
            Rerr_wtg(ixvarglaf_sk,1) = x0_wtg(ixvarglaf_sk,1);
...
%             Iswtgk = Iswtg(:,k2).*(cos(Vtangwtg) + 1i*sin(Vtangwtg));
            ...
            Efd_wtg(ixvarglaf_sk,k) = Rerr_wtg(ixvarglaf_sk,k);
...
%======================== Reactive Power Control =========================%
            ...
            ixvarglaf_m1_m2 = (varflag == -1) | (varflag == -2);
            dQset_wtg(ixvarglaf_m1_m2,k) = 0;
            % if varflag == 0 % case when it follows PF control           
            ixvarglaf_0 = varflag == 0;
            ixvarglaf_p3 = varflag == 3;% skip int
            ixvarglaf_0_p3 = ixvarglaf_0 | ixvarglaf_p3;

            ds6_wtg(ixvarglaf_0_p3,k) = (1./Tpwrwtg(ixvarglaf_0_p3,1)).*(Pg_wtg(ixvarglaf_0_p3,1) - s6_wtg(ixvarglaf_0_p3,k)); 
            Qset_wtg(ixvarglaf_0_p3,k) = (9/10)*s6_wtg(ixvarglaf_0_p3,k).*tan(PFArefwtg(ixvarglaf_0_p3,1)); % Accomodate bases
            dQset_wtg(ixvarglaf_0_p3,k) = 0; % So it's not overrided by s_simu
            % if varflag == 1 % case when it follows Voltage regulation
            ixvarglaf_p1 = varflag == 1;
            ixvarglaf_p2 = varflag == 2;
            ixvarglaf_p1_2 = ixvarglaf_p1 | ixvarglaf_p2;
