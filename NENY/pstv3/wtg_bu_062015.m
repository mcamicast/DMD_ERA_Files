...
% %         
            ...
            Vrfq = bus(bus_int(Vregbuswtg(ixvarglaf_p1)),2); % reference voltage magnitute from LF - checked in xxx--
            Vrfq = Vrfq(ixvarglaf_p1,1); % reference voltage taken from the load flow
            Vreg = bus(bus_int(Vregbuswtg(ixvarglaf_p1)),2); % Voltage to regulate is defined by user (from LF to initialize)
...
% ====================== End Freq signal filtering ====================== %            
            ...
            Vrfq_wtg(ixvarglaf_p1,1) = Vrfq; Vreg_wtg(ixvarglaf_p1,1) = Vreg;
            bus_new = bus;
...
%======================== Reactive Power Control =========================%
            ...
            % if varflag == 1 % case when it follows Voltage regulation
            ixvarglaf_p1 = varflag == 1;
            Vrfq = abs( busi_v(bus_int(Vregbuswtg(ixvarglaf_p1)),1) ); % reference voltage - checked in xxx--
            Vrfq = Vrfq(ixvarglaf_p1,1); % reference voltage taken from the load flow
            Vqd = Kqdwtg(ixvarglaf_p1,1).*s7_wtg(ixvarglaf_p1,k); % Figure 4-5 GE report v4.5
            Vreg = abs( busi_v(bus_int(Vregbuswtg(ixvarglaf_p1)),k) ); % Voltage to regulate is defined by user
            Vreg = Vreg(ixvarglaf_p1,1);
...
%             end
            ...
            Vrfq_wtg(ixvarglaf_p1,k) = Vrfq; Vreg_wtg(ixvarglaf_p1,k) = Vreg;
        end
