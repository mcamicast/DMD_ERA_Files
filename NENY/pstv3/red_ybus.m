...
global n_wtg wtg_idx Iswtg wtg2_con wtg3_con
global n_ccinj ccinj_idx
...
nwtg2 = size(wtg2_con,1); % number of induction generators
nwtg3 = size(wtg3_con,1); % number of generators
n_tot=n+nmot+nig+nwtg2+nwtg3;			% total number of machines
...
% subtract non-conforming loads from bus P and Q loads
  ...
  if nload~=0
    ...
    if n_wtg ~=0
        % in conjuntion with some lines below (Pl - bus_sol) the generation for WTG
        % becomes effectively zero.
        jwtg = j(wtg_idx); %jwtg = j;
        % Vwtg_nc = bus_sol(jwtg,2).*exp(jay*bus_sol(jwtg,3)*pi/180);
        % Swtg = Vwtg_nc.*conj(Iswtg(:,1));
        bus_sol(jwtg,6) = bus_sol(jwtg,4); % real(Swtg); % ;
        bus_sol(jwtg,7) = bus_sol(jwtg,5); % imag(Swtg); % ;
    end
    if n_ccinj ~=0
        % in conjuntion with some lines below (Pl - bus_sol) the generation
        % for CCINJ becomes effectively zero.
        jccinj = j(ccinj_idx);
        bus_sol(jccinj,6) = bus_sol(jccinj,4);
        bus_sol(jccinj,7) = bus_sol(jccinj,5);
    end
  end
end
...
if nig~=0
      ...
      y(ngm+1:n_tot-nwtg2,1) = ones(nig,1)./(rs+jay*xsp);     
      jm = bus_int(round(igen_con(:,2)));  % bus connected to induction generator
       ...
       % remove zero rows from permutaion matrix
       perm(jmz_idx,:) = [];
       Ymmod = diag(y(ngm+1:n_tot-nwtg2,1))*perm';
       Y_b(ngm+1:n_tot-nwtg2,jm) = -Ymmod;
       Y_d(jm,jm) = Y_d(jm,jm) + perm*Ymmod;
       igmax= max(igen_con(:,1));
end

% extract appropriate xsp from wtg type 2
% data
wtg2max=0;
if nwtg2~=0
    wtg2_ix = 1:nwtg2;
    ...
    Rswtg2 = Rswtg2.*basmva./genbasmva_wtg2; % BASE CHANGE TO SYSTEM BASE
    Xsp_wtg2 = Xsp_wtg2.*basmva./genbasmva_wtg2; % BASE CHANGE TO SYSTEM BASE
...
%       y(ngm+1:n_tot-nwtg2,1) = ones(nig,1)./(rs+jay*xsp);     
%       jm = bus_int(round(igen_con(:,2)));  % bus connected to induction generator
    y(n_tot-nwtg2+1:n_tot,1) = ones(nwtg2,1)./(Rswtg2+jay*Xsp_wtg2);  
    jm = bus_int(round(wtg2_con(:,2)));  % bus connected to WTG Type 2
    % check for multiple WTG type 2 generators at a bus
    perm = eye(nwtg2);
    for k = 1:nwtg2
        mm_idx = find(jm  == jm(k));
        lmm = length(mm_idx);
     ...
     % remove zero rows from permutaion matrix
     perm(jmz_idx,:) = [];
     Ymmod = diag(y(n_tot-nwtg2+1:n_tot,1))*perm';
     Y_b(n_tot-nwtg2+1:n_tot,jm) = -Ymmod;
     Y_d(jm,jm) = Y_d(jm,jm) + perm*Ymmod;
     wtg2max= max(wtg2_con(:,1));    
end
if (n_tot ~=0)
