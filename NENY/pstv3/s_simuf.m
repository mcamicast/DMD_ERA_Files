...
global wadc 
line = [];
wadc = 1; %cambiar para activar control
...
%% Load data of controllers tunning
%NENY_LQG_bvr_LIM_none; % Case 1: with-out control NENY
sint_era_DMD_lqg; % Case 2: With control
...
if isempty(dcsp_con)
  ...
  [bus_sol,line,line_flw] = ...
     loadflow(bus,line,tol,iter_max,0.5,'n',1);
...
%        gamma(:,:) = inv_par(:,1)*pi/180;
    for kkk = 1:n_dcl
        Vdc(r_idx(kkk),:) = rec_par(kkk,2)*ones(1,kdc); 
        Vdc(i_idx(kkk),:) = inv_par(kkk,2)*ones(1,kdc);
        i_dc(r_idx(kkk),:) = line_par(kkk)*ones(1,kdc);
        i_dc(i_idx(kkk),:) = line_par(kkk)*ones(1,kdc);
    end
     i_dcr(:,:) = i_dc(r_idx,:); i_dci(:,:) = i_dc(i_idx,:);
    for kkk = 1:n_dcl
        alpha(kkk,:) = ones(1,kdc)*rec_par(kkk,1)*pi/180;
        gamma(kkk,:) = ones(1,kdc)*inv_par(kkk,1)*pi/180;   
    end
%Final of HVDC links change 
   if ndcr_ud~=0
      for j = 1:ndcr_ud
         sv = get(dcr_dc{j,1});
         if j==1
...
%       vdp_wtg2(:,j) =  (:,k) + h_sol*(:,k);
      ...
      if t(j)>=1 
        mexc_sig_lqg_latence(t(j),j,flag);
      else
        mexc_sig_lqg(t(j),j);  
      end
...
sstr.cur_re=cur_re;sstr.cur_im=cur_im;% Mario
ccol = size(pmech,2);
...
line_range1 = 1;
        %             if isempty(line_range);line_range = 1:49;end
  sstr.V1_17 = bus_v(bus_int(line(line_range1,1)),:);
  sstr.V2_18 = bus_v(bus_int(line(line_range1,2)),:);
  R = line(line_range1,3); X = line(line_range1,4);
  B = line(line_range1,5);
  tap = line(line_range1,6); phi = line(line_range1,7);
  [sstr.SL1,S2] = line_pq(sstr.V1_17,sstr.V2_18,R,X,B,tap,phi);
  sstr.P_L1=real(sstr.SL1);%

  line_range16 = 16;
        %             if isempty(line_range);line_range = 1:49;end
  sstr.V1_24 = bus_v(bus_int(line(line_range16,1)),:);
  sstr.V2_25 = bus_v(bus_int(line(line_range16,2)),:);
  R = line(line_range16,3); X = line(line_range16,4);
  B = line(line_range16,5);
  tap = line(line_range16,6); phi = line(line_range16,7);
  [sstr.SL16,S2] = line_pq(sstr.V1_24,sstr.V2_25,R,X,B,tap,phi);
  sstr.P_L16=real(sstr.SL16);%

  line_range86 = 86;
  %             if isempty(line_range);line_range = 1:49;end
  sstr.V1_17 = bus_v(bus_int(line(line_range86,1)),:);
  sstr.V2_43 = bus_v(bus_int(line(line_range86,2)),:);
  R = line(line_range86,3); X = line(line_range86,4);
  B = line(line_range86,5);
  tap = line(line_range86,6); phi = line(line_range86,7);
  [sstr.SL86,S2] = line_pq(sstr.V1_17,sstr.V2_43,R,X,B,tap,phi);
  sstr.P_L86=real(sstr.SL86);%

  line_range73 = 73;
  %             if isempty(line_range);line_range = 1:49;end
  sstr.V1_66 = bus_v(bus_int(line(line_range73,1)),:);
  sstr.V2_68 = bus_v(bus_int(line(line_range73,2)),:);
  R = line(line_range73,3); X = line(line_range73,4);
  B = line(line_range73,5);
  tap = line(line_range73,6); phi = line(line_range73,7);
  [sstr.SL73,S2] = line_pq(sstr.V1_66,sstr.V2_68,R,X,B,tap,phi);
  sstr.P_L73=real(sstr.SL73);%

  line_range75 = 75;
  %             if isempty(line_range);line_range = 1:49;end
  sstr.V1_65 = bus_v(bus_int(line(line_range75,1)),:);
  sstr.V2_68 = bus_v(bus_int(line(line_range75,2)),:);
  R = line(line_range75,3); X = line(line_range75,4);
  B = line(line_range75,5);
  tap = line(line_range75,6); phi = line(line_range75,7);
  [sstr.SL75,S2] = line_pq(sstr.V1_65,sstr.V2_68,R,X,B,tap,phi);
  sstr.P_L75=real(sstr.SL75);%

  line_range76 = 76;
  %             if isempty(line_range);line_range = 1:49;end
  sstr.V1_68 = bus_v(bus_int(line(line_range76,1)),:);
  sstr.V2_58 = bus_v(bus_int(line(line_range76,2)),:);
  R = line(line_range76,3); X = line(line_range76,4);
  B = line(line_range76,5);
  tap = line(line_range76,6); phi = line(line_range76,7);
  [sstr.SL76,S2] = line_pq(sstr.V1_68,sstr.V2_58,R,X,B,tap,phi);
  sstr.P_L76=real(sstr.SL76);%  

  line_range77 = 77;
  %             if isempty(line_range);line_range = 1:49;end
  sstr.V1_58 = bus_v(bus_int(line(line_range77,1)),:);
  sstr.V2_57 = bus_v(bus_int(line(line_range77,2)),:);
  R = line(line_range77,3); X = line(line_range77,4);
  B = line(line_range77,5);
  tap = line(line_range77,6); phi = line(line_range77,7);
  [sstr.SL77,S2] = line_pq(sstr.V1_58,sstr.V2_57,R,X,B,tap,phi);
  sstr.P_L77=real(sstr.SL77);%    

  line_range78 = 78;
  %             if isempty(line_range);line_range = 1:49;end
  sstr.V1_57 = bus_v(bus_int(line(line_range78,1)),:);
  sstr.V2_56 = bus_v(bus_int(line(line_range78,2)),:);
  R = line(line_range78,3); X = line(line_range78,4);
  B = line(line_range78,5);
  tap = line(line_range78,6); phi = line(line_range78,7);
  [sstr.SL78,S2] = line_pq(sstr.V1_58,sstr.V2_57,R,X,B,tap,phi);
  sstr.P_L78=real(sstr.SL78);%    

%%

% line_range5 = 5;
% % if isempty(line_range);line_range = 1:49;end
% sstr.V1_3 = bus_v(bus_int(line(line_range5,1)),:);
% sstr.V2_101 = bus_v(bus_int(line(line_range5,2)),:);
% R = line(line_range5,3); X = line(line_range5,4);
% B = line(line_range5,5);
% tap = line(line_range5,6); phi = line(line_range5,7);
% [sstr.SL5,S2] = line_pq(sstr.V1_3,sstr.V2_101,R,X,B,tap,phi);
% sstr.P_L5=real(sstr.SL5);%
% 
% line_range6 = 6;
% % if isempty(line_range);line_range = 1:49;end
% sstr.V1_3 = bus_v(bus_int(line(line_range6,1)),:);
% sstr.V2_101 = bus_v(bus_int(line(line_range6,2)),:);
% R = line(line_range6,3); X = line(line_range6,4);
% B = line(line_range6,5);
% tap = line(line_range6,6); phi = line(line_range6,7);
% [sstr.SL6,S2] = line_pq(sstr.V1_3,sstr.V2_101,R,X,B,tap,phi);
% sstr.P_L6=real(sstr.SL6);%
% 
% line_range10 = 10;
% %  if isempty(line_range);line_range = 1:49;end
% sstr.V1_13 = bus_v(bus_int(line(line_range10,1)),:);
% sstr.V2_101 = bus_v(bus_int(line(line_range10,2)),:);
% R = line(line_range10,3); X = line(line_range10,4);
% B = line(line_range10,5);
% tap = line(line_range10,6); phi = line(line_range10,7);
% [sstr.SL10,S2] = line_pq(sstr.V1_13,sstr.V2_101,R,X,B,tap,phi);
% sstr.P_L10=real(sstr.SL10);%
% 
% line_range11 = 11;
% %  if isempty(line_range);line_range = 1:49;end
% sstr.V1_13 = bus_v(bus_int(line(line_range11,1)),:);
% sstr.V2_101 = bus_v(bus_int(line(line_range11,2)),:);
% R = line(line_range11,3); X = line(line_range11,4);
% B = line(line_range11,5);
% tap = line(line_range11,6); phi = line(line_range11,7);
% [sstr.SL11,S2] = line_pq(sstr.V1_13,sstr.V2_101,R,X,B,tap,phi);
% sstr.P_L11=real(sstr.SL11);%
