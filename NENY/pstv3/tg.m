...
global  tg1 tg2 tg3 tg4 dtg1 dtg2 dtg3 dtg4
...
if flag == 0; % initialization
   ...
   if i ~= 0  % scalar computation
      ...
      %
      tg_sig(i,1) = 0;
      % initialize aggressive governor state
      if tg_agg_f
        tg4(i,1) = 0;
      end
   else
      ...
      if n_tg~=0
         ...
         % initialize aggresive governor state
         if tg_agg_f
            tg4(tg_idx,1) = zeros(n_tg,1);
         end
      end
   end
end
...
if flag == 2 % turbine governor dynamics calculation
   if i ~= 0 % scalar computation
      ...
      if n_tg ~=0
         ...
         % insert agresive governing stage
         if tg_agg_f
             Kr_tg = tg_con(tg_idx,11);
             Tr_tg = tg_con(tg_idx,12);
             ixTr0 = Tr_tg == 0;
             Tr_tg(ixTr0) = 20;% random number to avoid dividing by 0
             dtg4(tg_idx,k) = (Kr_tg.*tg_con(tg_idx,4).*spd_err - tg4(tg_idx,k))./Tr_tg;
             auxdtg4 = dtg4(tg_idx,k); % set to 0 when Tr_tg = 0
             auxdtg4(ixTr0) = 0;       % set to 0 when Tr_tg = 0
             dtg4(tg_idx,k) = auxdtg4; % set to 0 when Tr_tg = 0
             
         else
             Tr_tg = 0;
         end
         demand = tg_pot(tg_idx,5) + spd_err.*tg_con(tg_idx,4) + tg_sig(tg_idx,k) + dtg4(tg_idx,k).*Tr_tg;
         % end insert agresive governing stage
