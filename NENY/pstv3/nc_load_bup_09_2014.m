...
if ~isempty(load_con)
   ...
   if flag == 1 % network interface computation 
      ...
      if n_wtg~=0
         ...
         Vwtg = V_nc(wtg_idx);
         Vangwtg = angle(Vwtg);
%          Swtg = Vwtg.*conj(Iswtg(:,kk));
%          load_wtg = Swtg./abs(Vwtg);
%          cur_wtg = conj((load_wtg.*abs(Vwtg))./Vwtg);
%          curr_nc(wtg_idx) = cur_wtg + curr_nc(wtg_idx);
         curr_nc(wtg_idx) = Iswtg(:,kk);
%          curr_nc(wtg_idx) = Iswtg(:,kk).*(cos(Vangwtg) + 1i*sin(Vangwtg));% + curr_nc(wtg_idx);
      end
      
      if n_ccinj~=0
          curr_nc(ccinj_idx) = Iccinj;
      end
