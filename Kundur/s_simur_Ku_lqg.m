...
plot_now=0;
jay = sqrt(-1);
pst_var_Ku % set up global variables 
svc_dc=[];tcsc_dc=[];dcr_dc=[];dci_dc=[];
% load input data from m.file
% Control_ku_15e_none; % Caso 1: sin control
% Control_Kundur_DMD_ERA
...
if n_wtg ~= 0
    zwwp = zeros(n_wtg,k);
    Iswtg = zwwp; Pgv_wtg = zwwp; Qgv_wtg = zwwp; Ipcmdv_wtg = zwwp; Pmechv_wtg = zwwp;
    Ipmxv_wtg = zwwp; Iqmxv_wtg = zwwp; Iqxv_wtg = zwwp;
    wref_wtg = zwwp; wgSV_wtg = zwwp; wtSV_wtg = zwwp; delg_wtg = zwwp; delt_wtg = zwwp; delerr1_wtg = zwwp; delerr2_wtg = zwwp;
    dwref_wtg = zwwp; dwgSV_wtg = zwwp; dwtSV_wtg = zwwp; ddelg_wtg = zwwp; ddelt_wtg = zwwp; ddelerr1_wtg = zwwp; ddelerr2_wtg = zwwp;
    ...
    s1wini_wtg = zwwp; s2wini_wtg = zwwp; ds1wini_wtg = zwwp; ds2wini_wtg = zwwp;
else
    zwwp = zeros(1,k);
    Iswtg = zwwp; Pgv_wtg = zwwp; Qgv_wtg = zwwp; Ipcmdv_wtg = zwwp; Pmechv_wtg = zwwp;
    Ipmxv_wtg = zwwp; Iqmxv_wtg = zwwp; Iqxv_wtg = zwwp;
...
freqcalc(k,t,0);
bus = wtg(0,1,bus,0);%initialize wtg OJO DEFINIR BIEN LA SALIDA confirmar bus
...
%       if t(k)>=1
%         mexc_sig_ku_lqg_latence(t(k),k,flag);mexc_sig_ku_lqg
%       else
        mexc_sig_ku_lqg(t(k),k);  
%       end  %Ojo, acá no se pone latencia por ser flag =1 
% Here does not uncomment this latence function, as flag is differrent to 1
...
%       if t(k)>=1
%         mexc_sig_ku_lqg_latence(t(k),k,flag);
%       else
        mexc_sig_ku_lqg(t(k),k);  
%       end

%Ojo acá tampoco va por estar en la etapa de predictor
% Here does not uncomment this latence function, as is the predictor stage
      ...
      tg_hydro(0,k,bus_sim,flag);
      if n_wtg~=0
         wtg(0,k,bus,flag); % ADDED - wtg DYNAMIC COMPUTATION
      end
      if n_svc~=0
...
%       end
      ...
      delg_wtg(:,j) = delg_wtg(:,k) + h_sol*ddelg_wtg(:,k);
      delt_wtg(:,j) = delt_wtg(:,k) + h_sol*ddelt_wtg(:,k);
      ...
      if t(k)>=1
        ...
        mexc_sig_ku_lqg(t(k),k);  
      end %Ojo acá va por ser flag 1
%here uncomment is you want to run the latence
      
      % mexc_sig_ku_lqg(t(j),j);
      ...
      if t(j)>=1 
        mexc_sig_ku_lqg_latence(t(j),j,flag);
      else
        mexc_sig_ku_lqg(t(j),j);  
      end
      %here uncomment is you want to run the latence
      ...
      tg_hydro(0,j,bus_sim,flag);
      if n_wtg~=0
         wtg(0,j,bus_sim,flag); % ADDED - wtg DYNAMIC COMPUTATION
      end
      if n_svc~=0
         msvc_sig(t(j),j);
         if n_dcud~=0
      ...
      delg_wtg(:,j) = delg_wtg(:,k) + h_sol*(ddelg_wtg(:,j) + ddelg_wtg(:,k))/2;
      delt_wtg(:,j) = delt_wtg(:,k) + h_sol*(ddelt_wtg(:,j) + ddelt_wtg(:,k))/2;
      ...
      s2wini_wtg(:,j) = s2wini_wtg(:,k) + h_sol*(ds2wini_wtg(:,j) + ds1wini_wtg(:,k))/2;
      %*** end add wtg states
      freqcalc(j,t,1); % method set to 0 by default
     
      %here uncomment is you want to run the latence
      %Acá se define la función para agregar la latencia.
      if t(j)>=1 
        mexc_sig_ku_lqg_latence(t(j),j,flag);
      else
        mexc_sig_ku_lqg(t(j),j);  
...
line_range5 = 5;
% if isempty(line_range);line_range = 1:49;end
V1_3 = bus_v(bus_int(line(line_range5,1)),:);
V2_101 = bus_v(bus_int(line(line_range5,2)),:);
R = line(line_range5,3); X = line(line_range5,4);
B = line(line_range5,5);
tap = line(line_range5,6); phi = line(line_range5,7);
[SL5,S2] = line_pq(V1_3,V2_101,R,X,B,tap,phi);
P_L5=real(SL5);%

line_range6 = 6;
% if isempty(line_range);line_range = 1:49;end
V1_3 = bus_v(bus_int(line(line_range6,1)),:);
V2_101 = bus_v(bus_int(line(line_range6,2)),:);
R = line(line_range6,3); X = line(line_range6,4);
B = line(line_range6,5);
tap = line(line_range6,6); phi = line(line_range6,7);
[SL6,S2] = line_pq(V1_3,V2_101,R,X,B,tap,phi);
P_L6=real(SL6);%

line_range10 = 10;
%  if isempty(line_range);line_range = 1:49;end
V1_13 = bus_v(bus_int(line(line_range10,1)),:);
V2_101 = bus_v(bus_int(line(line_range10,2)),:);
R = line(line_range10,3); X = line(line_range10,4);
B = line(line_range10,5);
tap = line(line_range10,6); phi = line(line_range10,7);
[SL10,S2] = line_pq(V1_13,V2_101,R,X,B,tap,phi);
P_L10=real(SL10);%

line_range11 = 11;
%  if isempty(line_range);line_range = 1:49;end
V1_13 = bus_v(bus_int(line(line_range11,1)),:);
V2_101 = bus_v(bus_int(line(line_range11,2)),:);
R = line(line_range11,3); X = line(line_range11,4);
B = line(line_range11,5);
tap = line(line_range11,6); phi = line(line_range11,7);
[SL11,S2] = line_pq(V1_13,V2_101,R,X,B,tap,phi);
P_L11=real(SL11);%
  
%% ---------------------------------------------------
%   line_range73 = 73;
%   %             if isempty(line_range);line_range = 1:49;end
%   V1_66 = bus_v(bus_int(line(line_range73,1)),:);
%   V2_68 = bus_v(bus_int(line(line_range73,2)),:);
%   R = line(line_range73,3); X = line(line_range73,4);
%   B = line(line_range73,5);
%   tap = line(line_range73,6); phi = line(line_range73,7);
%   [SL73,S2] = line_pq(V1_66,V2_68,R,X,B,tap,phi);
%   P_L73=real(SL73);%
% 
%   line_range75 = 75;
%   %             if isempty(line_range);line_range = 1:49;end
%   V1_65 = bus_v(bus_int(line(line_range75,1)),:);
%   V2_68 = bus_v(bus_int(line(line_range75,2)),:);
%   R = line(line_range75,3); X = line(line_range75,4);
%   B = line(line_range75,5);
%   tap = line(line_range75,6); phi = line(line_range75,7);
%   [SL75,S2] = line_pq(V1_65,V2_68,R,X,B,tap,phi);
%   P_L75=real(SL75);%
% 
%   line_range76 = 76;
%   %             if isempty(line_range);line_range = 1:49;end
%   V1_68 = bus_v(bus_int(line(line_range76,1)),:);
%   V2_58 = bus_v(bus_int(line(line_range76,2)),:);
%   R = line(line_range76,3); X = line(line_range76,4);
%   B = line(line_range76,5);
%   tap = line(line_range76,6); phi = line(line_range76,7);
%   [SL76,S2] = line_pq(V1_68,V2_58,R,X,B,tap,phi);
%   P_L76=real(SL76);%  
%   
%   line_range77 = 77;
%   %             if isempty(line_range);line_range = 1:49;end
%   V1_58 = bus_v(bus_int(line(line_range77,1)),:);
%   V2_57 = bus_v(bus_int(line(line_range77,2)),:);
%   R = line(line_range77,3); X = line(line_range77,4);
%   B = line(line_range77,5);
%   tap = line(line_range77,6); phi = line(line_range77,7);
%   [SL77,S2] = line_pq(V1_58,V2_57,R,X,B,tap,phi);
%   P_L77=real(SL77);%    
%   
%   line_range78 = 78;
%   %             if isempty(line_range);line_range = 1:49;end
%   V1_57 = bus_v(bus_int(line(line_range78,1)),:);
%   V2_56 = bus_v(bus_int(line(line_range78,2)),:);
%   R = line(line_range78,3); X = line(line_range78,4);
%   B = line(line_range78,5);
%   tap = line(line_range78,6); phi = line(line_range78,7);
%   [SL78,S2] = line_pq(V1_58,V2_57,R,X,B,tap,phi);
%   P_L78=real(SL78);%    
