...
global n_wtg2 vdp_wtg2 vqp_wtg2 id_wtg2 iq_wtg2 wtg2_con P_wtg2 Q_wtg2
...
%  vmpwtg2 = vdp_wtg2(:,k) + jay*vqp_wtg2(:,k); %%Romel
 if (n_mot~=0&n_ig==0)
    ntot = n_mac + n_mot;
    ngm = n_mac + n_mot;
 ...
 elseif  (n_mot~=0&n_ig~=0)
    ...
    ntot = n_mac; % added FWB
    int_volt = psi;
 end
 if n_wtg2~=0;
     ntot = ntot+n_wtg2;
%      ngm = ?? %probably not needed
     int_volt = [int_volt;vmpwtg2];
 end
 ...
 if nload~=0
   if k~=1; kk = k-1;else; kk=k;end; %kk=k; % just added 8-13-2013
 ...
 if n_ig~=0
    idig(:,k) = -real(cur(ngm+1:ntot-n_wtg2));%induction generator currents
    iqig(:,k) = -imag(cur(ngm+1:ntot-n_wtg2));%current out of network
...
if n_wtg2~=0
    id_wtg2(:,k) = -real(cur(ntot-n_wtg2+1:ntot));%WTG Type 2 currents(in system base)
    iq_wtg2(:,k) = -imag(cur(ntot-n_wtg2+1:ntot));%current out of network
    S_wtg2(:,k) = bus_v(bus_int(wtg2_con(:,2)),k).*(id_wtg2(:,k) - jay*iq_wtg2(:,k));
    P_wtg2(:,k) = real(S_wtg2(:,k));
    Q_wtg2(:,k) = imag(S_wtg2(:,k));
end
