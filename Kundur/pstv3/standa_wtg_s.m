function [sstr] = standa_wtg_s(dfile,pathname,sys_freq_i,basmva_i)
%
% Inputs:
% vtwtg_pr -wind profile
% t -time profile
% S0 -initial apparent power
% wtg_con
clear global
pst_var % set up global variables 
pst_var_sens; % rest of global variables for tr. sensitivity calculation
line = [];
...
%*** end add wtg
%*** add wtg vars for tr. sensitivity comp.
dEfd_dkVi_swtg = zwwp; dEfd_dkQi_swtg = zwwp; dRerr_dkQi_swtg = zwwp; dEfd_dkiv_swtg = zwwp; dRerr_dkiv_swtg = zwwp;
dQord_dkiv_swtg = zwwp; dEfd_dkpv_swtg = zwwp; dRerr_dkpv_swtg = zwwp; dQord_dkpv_swtg = zwwp;
ddEfd_dkVi_swtg = zwwp; ddEfd_dkQi_swtg = zwwp; ddRerr_dkQi_swtg = zwwp; ddEfd_dkiv_swtg = zwwp; ddRerr_dkiv_swtg = zwwp;
ddQord_dkiv_swtg = zwwp; ddEfd_dkpv_swtg = zwwp; ddRerr_dkpv_swtg = zwwp; ddQord_dkpv_swtg = zwwp;

dx0_dkVi_swtg = zwwp; dx0_dkQi_swtg = zwwp; dx0_dkiv_swtg = zwwp; dx0_dkpv_swtg = zwwp;
ddx0_dkVi_swtg = zwwp; ddx0_dkQi_swtg = zwwp; ddx0_dkiv_swtg = zwwp; ddx0_dkpv_swtg = zwwp;

dIsre_dkVi_swtg = zwwp; dIsre_dkQi_swtg = zwwp; dIsre_dkiv_swtg = zwwp; dIsre_dkpv_swtg = zwwp;
dIsim_dkVi_swtg = zwwp; dIsim_dkQi_swtg = zwwp; dIsim_dkiv_swtg = zwwp; dIsim_dkpv_swtg = zwwp;
%*** end add wtg vars for tr. sensitivity comp.
...
%     h_sol = dt(k-1);
    ...
    if n_wtg~=0
        ...
        wtg_sens(0,k,bus_sim,flag,bus_v);
    end
    ...
    % *** add wtg param sensitivities
    dEfd_dkVi_swtg(:,j) = dEfd_dkVi_swtg(:,j) + h_sol*ddEfd_dkVi_swtg(:,k);
    dEfd_dkQi_swtg(:,j) = dEfd_dkQi_swtg(:,j) + h_sol*ddEfd_dkQi_swtg(:,k);
    dRerr_dkQi_swtg(:,j) = dRerr_dkQi_swtg(:,j) + h_sol*ddRerr_dkQi_swtg(:,k);
    dEfd_dkiv_swtg(:,j) = dEfd_dkiv_swtg(:,j) + h_sol*ddEfd_dkiv_swtg(:,k);
    dRerr_dkiv_swtg(:,j) = dRerr_dkiv_swtg(:,j) + h_sol*ddRerr_dkiv_swtg(:,k);
    dQord_dkiv_swtg(:,j) = dQord_dkiv_swtg(:,j) + h_sol*ddQord_dkiv_swtg(:,k);
    dEfd_dkpv_swtg(:,j) = dEfd_dkpv_swtg(:,j) + h_sol*ddEfd_dkpv_swtg(:,k);
    dRerr_dkpv_swtg(:,j) = dRerr_dkpv_swtg(:,j) + h_sol*ddRerr_dkpv_swtg(:,k);
    dQord_dkpv_swtg(:,j) = dQord_dkpv_swtg(:,j) + h_sol*ddQord_dkpv_swtg(:,k);
    
    dx0_dkVi_swtg(:,j) = dx0_dkVi_swtg(:,j) + h_sol*ddx0_dkVi_swtg(:,k);
    dx0_dkQi_swtg(:,j) = dx0_dkQi_swtg(:,j) + h_sol*ddx0_dkQi_swtg(:,k);
    dx0_dkiv_swtg(:,j) = dx0_dkiv_swtg(:,j) + h_sol*ddx0_dkiv_swtg(:,k);
    dx0_dkpv_swtg(:,j) = dx0_dkpv_swtg(:,j) + h_sol*ddx0_dkpv_swtg(:,k);
    % *** add wtg param sensitivities
    ...
    if n_wtg~=0
        ...
        wtg_sens(0,j,bus_sim,flag,bus_v);
    end
    %*** add wtg states
    ...
    % *** add wtg param sensitivities
    dEfd_dkVi_swtg(:,j) = dEfd_dkVi_swtg(:,j) + h_sol*(ddEfd_dkVi_swtg(:,j) + ddEfd_dkVi_swtg(:,k))/2;
    dEfd_dkQi_swtg(:,j) = dEfd_dkQi_swtg(:,j) + h_sol*(ddEfd_dkQi_swtg(:,j) + ddEfd_dkQi_swtg(:,k))/2;
    dRerr_dkQi_swtg(:,j) = dRerr_dkQi_swtg(:,j) + h_sol*(ddRerr_dkQi_swtg(:,j) + ddRerr_dkQi_swtg(:,k))/2;
    dEfd_dkiv_swtg(:,j) = dEfd_dkiv_swtg(:,j) + h_sol*(ddEfd_dkiv_swtg(:,j) + ddEfd_dkiv_swtg(:,k))/2;
    dRerr_dkiv_swtg(:,j) = dRerr_dkiv_swtg(:,j) + h_sol*(ddRerr_dkiv_swtg(:,j) + ddRerr_dkiv_swtg(:,k))/2;
    dQord_dkiv_swtg(:,j) = dQord_dkiv_swtg(:,j) + h_sol*(ddQord_dkiv_swtg(:,j) + ddQord_dkiv_swtg(:,k))/2;
    dEfd_dkpv_swtg(:,j) = dEfd_dkpv_swtg(:,j) + h_sol*(ddEfd_dkpv_swtg(:,j) + ddEfd_dkpv_swtg(:,k))/2;
    dRerr_dkpv_swtg(:,j) = dRerr_dkpv_swtg(:,j) + h_sol*(ddRerr_dkpv_swtg(:,j) + ddRerr_dkpv_swtg(:,k))/2;
    dQord_dkpv_swtg(:,j) = dQord_dkpv_swtg(:,j) + h_sol*(ddQord_dkpv_swtg(:,j) + ddQord_dkpv_swtg(:,k))/2;
    
    dx0_dkVi_swtg(:,j) = dx0_dkVi_swtg(:,j) + h_sol*(ddx0_dkVi_swtg(:,j) + ddx0_dkVi_swtg(:,k))/2;
    dx0_dkQi_swtg(:,j) = dx0_dkQi_swtg(:,j) + h_sol*(ddx0_dkQi_swtg(:,j) + ddx0_dkQi_swtg(:,k))/2;
    dx0_dkiv_swtg(:,j) = dx0_dkiv_swtg(:,j) + h_sol*(ddx0_dkiv_swtg(:,j) + ddx0_dkiv_swtg(:,k))/2;
    dx0_dkpv_swtg(:,j) = dx0_dkpv_swtg(:,j) + h_sol*(ddx0_dkpv_swtg(:,j) + ddx0_dkpv_swtg(:,k))/2;
    % *** add wtg param sensitivities
