...
if k==1 % initialization
    ...
    if size(wtg_con,2)>75
        ...
        ixfcwtg12 = fcflagwtg == 12; % G_f_Pth - damping control
        ixfcwtg17 = fcflagwtg == 17; % G_Il_QQ - damping control
        ixfcwtg20 = fcflagwtg == 20; % G_P_QQ - damping control
...
%         thrshldnfc(ixfcwtg2,:) = 1; % switching control variable for sw = 1
        ...
        if any(p3fcwtg(ixfcwtg4,1) ==0)
            ...
            ixCdiff = ixfcwtg2 | ixfcwtg12 | ixfcwtg17 | ixfcwtg20;
            p2_0 = (p2fcwtg ==0) & (ixCdiff);
            p4_0 = (p4fcwtg ==0) & (ixCdiff);
...
%         ixlin = p7fcwtg(ixfcwtg17 | ixfcwtg20);
%         
%         V1_fl = V1(ixlin,k);
%         V2_fl = V2(ixlin,k);
%         R_fl = squeeze(line_compl(ixlin,3,k));
%         X_fl = squeeze(line_compl(ixlin,4,k));
%         B_fl = squeeze(line_compl(ixlin,5,k));
%         tap_fl = squeeze(line_compl(ixlin,6,k));
%         phi_fl = squeeze(line_compl(ixlin,7,k));
...
%         Ilm_DCsig = abs(Il_f);
%         
%         Usig0 = zeros(size(p2fcwtg));
%         Usig0(ixfcwtg17,k) = Ilm_DCsig;
%         Usig0(ixfcwtg20,k) = P_DCsig;
    end
...
%         ixbferr_wtg = (freqrefwtg - swinif_wtg(:,k))<dbfcwtg; % only consider dips in freq
        ...
        maxline = size(line,1);
        if any( p7fcwtg(ixfcwtg17 | ixfcwtg20) > maxline )
        ...
        V1_fl = V1(ixlin,1);
        V2_fl = V2(ixlin,1);
        R_fl = squeeze(line_compl(ixlin,3,1));
        X_fl = squeeze(line_compl(ixlin,4,1));
        B_fl = squeeze(line_compl(ixlin,5,1));
        tap_fl = squeeze(line_compl(ixlin,6,1));
        phi_fl = squeeze(line_compl(ixlin,7,1));
        [Sf_fl,St_fl] = line_pqf(V1_fl,V2_fl,R_fl,X_fl,B_fl,tap_fl,phi_fl);
        P_DCsig = real(Sf_fl);
        ...
        Usig0 = zeros(size(p2fcwtg));
        Usig0(ixfcwtg17,1) = Ilm_DCsig;
        Usig0(ixfcwtg20,1) = P_DCsig;
        ...
        V1_fl = V1(ixlin,kk);
        V2_fl = V2(ixlin,kk);
        R_fl = squeeze(line_compl(ixlin,3,kk));
        X_fl = squeeze(line_compl(ixlin,4,kk));
        B_fl = squeeze(line_compl(ixlin,5,kk));
        tap_fl = squeeze(line_compl(ixlin,6,kk));
        phi_fl = squeeze(line_compl(ixlin,7,kk));
        [Sf_fl,St_fl] = line_pqf(V1_fl,V2_fl,R_fl,X_fl,B_fl,tap_fl,phi_fl);
        P_DCsig = real(Sf_fl);
        ...
        Usigk = zeros(size(p2fcwtg));
        Usigk(ixfcwtg17,k) = Ilm_DCsig;
        Usigk(ixfcwtg20,k) = P_DCsig;
        
        Usigerr = Usig0 - Usigk(:,k);
        end
        % sigpru5_wtg(:,k) = Ilm_DCsig;
...
% Kp + Ki/s
            ...
            Pth_sig_wtg(ixfcwtg1,k) = -sigpru1_wtg(ixfcwtg1,k);
...
% Output y - sigpru1_wtg(ixfcwtg3,k)
            ...
            ixCdiff = ixfcwtg2 | ixfcwtg12 | ixfcwtg17 | ixfcwtg20;
            ixCNdiff = ixfcwtg12 | ixfcwtg17 | ixfcwtg20;
            p2_0 = (p2fcwtg ==0) & (ixCdiff);
            p4_0 = (p4fcwtg ==0) & (ixCdiff);
            p6_0 = (p6fcwtg ==0) & (ixfcwtg2);
...
%             end
            ...
            Usig_in = zeros(size(p2fcwtg));
            Usig_in(ixCdiff,1) = busferr_wtg(ixCdiff,1);
            
            Usig_in(ixfcwtg17,1) = Usigerr(ixfcwtg17,1); % Current error sig.
            Usig_in(ixfcwtg20,1) = Usigerr(ixfcwtg20,1); % Power error sig.
            
            ixdstage = ixCdiff; % include the power and the other signals
...
%             rsigint(p2_0,1) = busferr_wtg(p2_0,1);
            ...
            % First Stage - Methods: 2, 12, 17, 20
            rsigint = (fst1_wtg(:,k) + p1fcwtg(:,1).*Usig_in(:,1))./p2fcwtg(:,1);
            dfst1_wtg(ixdstage,k) = ( Usig_in(ixdstage,1) - rsigint(ixdstage,1) );
            rsigint(p2_0,1) = Usig_in(p2_0,1);

            % Second Stage - Methods: 2, 12, 17, 20
            rsigint2 = (fst2_wtg(:,k) + p3fcwtg(:,1).*rsigint)./p4fcwtg(:,1);
            dfst2_wtg(ixdstage,k) = ( rsigint(ixdstage,1) - rsigint2(ixdstage,1) );
            rsigint2(p4_0,1) = rsigint(p4_0,1);
            
            % Third Stage - Method: 2 Only (so far)
            ...
            % Washout - Methods: 12, 17, 20
            dfst3_wtg(ixCNdiff,k) = rsigint2(ixCNdiff,1) - fst3_wtg(ixCNdiff,k)./p5fcwtg(ixCNdiff,1);
            sigpru1_wtg(ixCNdiff,k) = p6fcwtg(ixCNdiff,1).*dfst3_wtg(ixCNdiff,k);
            
            Pth_sig_wtg(ixfcwtg2 | ixfcwtg12,k) = -sigpru1_wtg(ixfcwtg2 | ixfcwtg12,k);
            
            QQ_sig_wtg(ixfcwtg20 | ixfcwtg17,k) = sigpru1_wtg(ixfcwtg20 | ixfcwtg17,k);
...
% Tr   - p3fcwtg
            ...
            Pth_sig_wtg(ixfcwtg3,k) = -sigpru1_wtg(ixfcwtg3,k);
...
%=================== assign on variable =================%
            ...
            % Pth_sig_wtg(:,k) = -sigpru1_wtg(:,k); % CAMBIAR ESTA ASIGNACIÓN INCLUYENDO INDICES
