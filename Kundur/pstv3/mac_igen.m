...
if ~isempty(igen_con)
   if flag == 0;
      ...
      if i == 0;
         ...
         igen_pot(:,4)=igen_con(:,8)+igen_con(:,6);%Xrr
         ...
         vdpig = real(vp); % error - fbw - I think it should be vdpig(:,1) = real(vp);
         vqpig = imag(vp); % error - fbw - I think it should be vqpig(:,1) = imag(vp);
