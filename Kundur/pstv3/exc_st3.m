...
global  dEfd dV_R dV_As dR_f dV_TR % borre Efd
global  exc_sig
...
%                                          % voltage
      nosat_idx = find(eqprime(n,k)<.8); %% error FWB thinks should be index k instead of 1
      if ~isempty(nosat_idx)
        E_Isat(nosat_idx) = eqprime(n(nosat_idx),k);
      end
