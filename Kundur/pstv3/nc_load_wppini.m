...
% wpp wariables
global wpp_con n_wpp Iswpp wpp_idx
...
%       if n_wpp ~= 0
%          Vwpp_nc =V_nc(wpp_idx);
%          Swpp = Vwpp_nc.*conj(Iswpp(:,1)); 
%          load_pot(wpp_idx,2) = -Swpp./abs(Vwpp_nc);
%          load_pot(wpp_idx,4) = -Swpp./Vwpp_nc./conj(Vwpp_nc);
%          % load_pot(wpp_idx,4) = -conj(Swpp)./Vwpp_nc./conj(Vwpp_nc);
%       end
...
%       end
      ...
      if (n_wpp~=0) % modify index - temporary
          if (abs(Y21)<1e-5)
             curr_mis = 0; 
             V_nc = inv(Y22)*Iswpp(:,kk);
          end
      end
      count = 0;
      Y22_real = real(Y22); Y22_imag = imag(Y22); pruebar{1}=[V_nc];pruebar{2}=[psi];
      % Newton's algorithm
      while(norm(curr_mis,'inf') > tol)
         ...
         % update voltage
         count = count + 1; pruebar{1}=[pruebar{1} V_nc]; pruebar{2}=[pruebar{2} psi];
         lv_idx = find(abs(V_nc)<=0.5); % lv_idx = find(abs(V_nc)<=0.5);
         hv_idx = find(abs(V_nc) > 0.5); % hv_idx = find(abs(V_nc) > 0.5);
         curr_load = Y21*psi + Y22*V_nc;
         if ~isempty(hv_idx)
...
%          if (n_wpp~=0)&&(fllv == 1)
%              % modify index - temporary
%              curr_mis(lv_idx) = Iswpp(:,kk) - curr_load(lv_idx); 
%          end
         if n_conv~=0
